...
$colors = $wc->random_palette(count => 10);
is( scalar @$colors, 10, 'Right number of colors with count' );

$colors = $wc->random_palette(count => 10, saturation => 0.8);
is( scalar @$colors, 10, 'Right number of colors with saturation' );

$colors = $wc->random_palette(count => 10, value => 0.8);
is( scalar @$colors, 10, 'Right number of colors with value' );

$colors = $wc->random_palette(count => 10, saturation => 0.8, value => 0.2);
is( scalar @$colors, 10, 'Right number of colors with saturation and value' );

# Check for death with bad parameters
dies_ok( sub { $wc->random_palette(count => 'flurb') }, 									'Dies on non-integer number of colors' );
dies_ok( sub { $wc->random_palette(count => 10, saturation => 'flurb') }, 'Dies on non-integer saturation' );
dies_ok( sub { $wc->random_palette(count => 10, value => 'flurb') }, 			'Dies on non-integer value' );